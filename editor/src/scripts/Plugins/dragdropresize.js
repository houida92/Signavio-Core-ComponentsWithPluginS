...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
	...
	handleMouseUp: function(event) {
		...
		if (selectedShape.getStencil().title() == 'Configurable Inclusive Gateway'
				|| selectedShape.getStencil().title() == 'Configurable Data-based exclusive (XOR) gateway'
				|| selectedShape.getStencil().title() == 'Configurable Parallel Gateway') {
			// Get all shapes
 					...
 					if (allShapes[j].resourceId == selectedOutgoingId) {
						...
						for (var k = 0; k < allShapes.length; ++k) {
 							if (allShapes[k].resourceId == targetShapeId) {
 								if(nbelements>0)
 									elements += ", ";
 								if(typeof allShapes[k].properties.gatewaytype === 'undefined')
 									elements += "{\"name\":\""+allShapes[k].properties.name+"\",\"actif\":\"true\"}";
 								else
 									elements += "{\"name\":\""+allShapes[k].properties.gatewaytype+"\",\"actif\":\"true\"}";
 								nbelements++;
 							}
						}
 					}
			...
			targets = "{\"totalCount\":"+nbelements+","+elements+"]}",
			// Set property of selected shape
			selectedShape.properties['oryx-outputconfiguration'] = targets;

			
			// Get incoming control flows' id of the selected shape
			var selectedIncoming = this.currentShapes[0].incoming;
			var targets1 = "";
			var elements1 = "\"items\":[";
			var nbelements1 = 0;
			
			for (var i = 0; i < selectedIncoming.length; ++i) {
				
				// Get control flow shapes by outgoing control flows' id
				var selectedIncomingId = selectedIncoming[i].resourceId;

				for (var j = 0; j < allShapes.length; ++j) {
					
					for(var k = 0 ; k < allShapes[j].outgoing.length ; ++k) {
						
						if (allShapes[j].outgoing[k].resourceId == selectedIncomingId) {
	 						
							if(nbelements1>0)
	 							elements1 += ", ";
							if(typeof allShapes[j].properties.gatewaytype === 'undefined')
								elements1 += "{\"name\":\""+allShapes[j].properties.name+"\",\"actif\":\"true\"}";
							else
								elements1 += "{\"name\":\""+allShapes[j].properties.gatewaytype+"\",\"actif\":\"true\"}";
							nbelements1++;
	 					}
					} 					
				}
			...
			targets1 = "{\"totalCount\":"+nbelements1+","+elements1+"]}",
			// Set property of selected shape
			selectedShape.properties['oryx-inputconfiguration'] = targets1;
