...
import de.hpi.bpmn2_0.model.activity.ConfigurableTask;
...
public class Diagram2BpmnConverter {
	...
	public final static HashSet<String> edgeIds = new HashSet<String>(
			Arrays.asList(edgeIdsArray));
	...
	private static Logger logger = Logger
			.getLogger(Diagram2BpmnConverter.class);
	...
	private BPMNElement createBpmnElementsRecursively(GenericShape<?, ?> shape)
		...
		for (GenericShape childShape : shape.getChildShapesReadOnly()) {
			BPMNElement childEl = this
					.createBpmnElementsRecursively(childShape);
			if (childEl != null) {
	...
	private void detectConnectors() {
		for (GenericEdge<?, ?> edge : DiagramHelper.getAllEdges(this.diagram)) {
			...
			BPMNElement target = (edge.getTarget() != null) ? this.bpmnElements
					.get(edge.getTarget().getResourceId()) : null;
	...
	private void updateDataAssociationsRefs() {
		...
		HashSet<String> associationIds = new HashSet<String>(
				Arrays.asList(associationIdsArray));
		...
		for (GenericShape shape : this.diagram.getAllShapesReadOnly()) {
			...
			if (dataAssociation instanceof DataInputAssociation) {
				...
				if (relatedActivity != null) {
					...
					bpmnConnector.setNode(new Association(
							(DataInputAssociation) dataAssociation));
				}
	...
	private void handleDataObjects() {
		...
		for (AbstractDataObject dataObject : dataObjects) {
			...
			if (dataObject.getProcess() == null && this.processes.size() > 0) {
				dataObject
						.setProcess(this.processes.get(this.processes.size() - 1));
	...
	private void handleStorages() {
		...
		for (Storage storage : storages) {
			...
			if (storage.getProcess() == null && this.processes.size() > 0) {
				storage.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(storage);
	...
	/**
	 * Assigns the Computes to the appropriate {@link Process}.
	 */
	private void handleComputes() {
		ArrayList<Compute> computes = new ArrayList<Compute>();
		this.getAllComputes(this.diagramChilds, computes);

		for (Compute compute : computes) {
			if (compute.getProcess() != null)
				continue;

			if (compute.getProcess() == null && this.processes.size() > 0) {
				compute.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(compute);
			} else if (compute.getProcess() == null) {
				Process process = new Process();
				this.processes.add(process);
				process.setId(SignavioUUID.generate());
				process.addChild(compute);
				compute.setProcess(process);
			}
	...
	/**
	 * Retrieves all Compute elements.
	 * 
	 * @param elements
	 *            The list of {@link BPMNElement}.
	 * 
	 * @param computes
	 *            The resulting list of {@link Compute}
	 */
	private void getAllComputes(List<BPMNElement> elements,
			List<Compute> computes) {
		for (BPMNElement element : elements) {
			if (element.getNode() instanceof Lane
					|| element.getNode() instanceof SubProcess) {
				getAllComputes(this.getChildElements(element), computes);
				continue;
			}

			if (element.getNode() instanceof Compute) {
				computes.add((Compute) element.getNode());
			}
		}
	}

	/**
	 * Assigns the Networks to the appropriate {@link Process}.
	 */
	private void handleNetworks() {
		ArrayList<Network> networks = new ArrayList<Network>();
		this.getAllNetworks(this.diagramChilds, networks);

		for (Network network : networks) {
			if (network.getProcess() != null)
				continue;

			if (network.getProcess() == null && this.processes.size() > 0) {
				network.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(network);
			} else if (network.getProcess() == null) {
				Process process = new Process();
				this.processes.add(process);
				process.setId(SignavioUUID.generate());
				process.addChild(network);
	...
	/**
	 * Retrieves all Network elements.
	 * 
	 * @param elements
	 *            The list of {@link BPMNElement}.
	 * 
	 * @param Networks
	 *            The resulting list of {@link Network}
	 */
	private void getAllNetworks(List<BPMNElement> elements,
			List<Network> networks) {
		for (BPMNElement element : elements) {
			if (element.getNode() instanceof Lane
					|| element.getNode() instanceof SubProcess) {
				getAllNetworks(this.getChildElements(element), networks);
				continue;
			}

			if (element.getNode() instanceof Network) {
				networks.add((Network) element.getNode());
			}
		}
	}
	
	/**
	 * Assigns the Assignments to the appropriate {@link Process}.
	 */
	private void handleAssignments() {
		ArrayList<Assignment> assignments = new ArrayList<Assignment>();
		this.getAllAssignments(this.diagramChilds, assignments);

		for (Assignment assignment : assignments) {
			if (assignment.getProcess() != null)
				continue;

			if (assignment.getProcess() == null && this.processes.size() > 0) {
				assignment.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(assignment);
			} else if (assignment.getProcess() == null) {
				Process process = new Process();
				this.processes.add(process);
				process.setId(SignavioUUID.generate());
	...
	/**
	 * Retrieves all assignment elements.
	 * 
	 * @param elements
	 *            The list of {@link BPMNElement}.
	 * 
	 * @param Assignment
	 *            The resulting list of {@link Assignment}
	 */
	private void getAllAssignments(List<BPMNElement> elements,
			List<Assignment> assignments) {
		for (BPMNElement element : elements) {
			if (element.getNode() instanceof Lane
					|| element.getNode() instanceof SubProcess) {
				getAllAssignments(this.getChildElements(element), assignments);
				continue;
			}

			if (element.getNode() instanceof Assignment) {
				assignments.add((Assignment) element.getNode());
			}
		}
	}
	
	/**
	 * Assigns the Elasticitys to the appropriate {@link Process}.
	 */
	private void handleElasticitys() {
		ArrayList<Elasticity> elasticitys = new ArrayList<Elasticity>();
		this.getAllElasticitys(this.diagramChilds, elasticitys);

		for (Elasticity elasticity : elasticitys) {
			if (elasticity.getProcess() != null)
				continue;

			if (elasticity.getProcess() == null && this.processes.size() > 0) {
				elasticity.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(elasticity);
			} else if (elasticity.getProcess() == null) {
				Process process = new Process();
				this.processes.add(process);
				process.setId(SignavioUUID.generate());
				process.addChild(elasticity);
				elasticity.setProcess(process);
			}
	...
	/**
	 * Retrieves all elasticity elements.
	 * 
	 * @param elements
	 *            The list of {@link BPMNElement}.
	 * 
	 * @param Elasticity
	 *            The resulting list of {@link Elasticity}
	 */
	private void getAllElasticitys(List<BPMNElement> elements,
			List<Elasticity> elasticitys) {
		for (BPMNElement element : elements) {
			if (element.getNode() instanceof Lane
					|| element.getNode() instanceof SubProcess) {
				getAllElasticitys(this.getChildElements(element), elasticitys);
				continue;
			}

			if (element.getNode() instanceof Elasticity) {
				elasticitys.add((Elasticity) element.getNode());
			}
		}
	}
	...
	/**
	 * Assigns the Sharingbatchings to the appropriate {@link Process}.
	 */
	private void handleSharingbatchings() {
		ArrayList<Sharingbatching> sharingbatchings = new ArrayList<Sharingbatching>();
		this.getAllSharingbatchings(this.diagramChilds, sharingbatchings);

		for (Sharingbatching sharingbatching : sharingbatchings) {
			if (sharingbatching.getProcess() != null)
				continue;

			if (sharingbatching.getProcess() == null && this.processes.size() > 0) {
				sharingbatching.setProcess(this.processes.get(this.processes.size() - 1));
				this.processes.get(this.processes.size() - 1).addChild(sharingbatching);
			} else if (sharingbatching.getProcess() == null) {
				Process process = new Process();
				this.processes.add(process);
				process.setId(SignavioUUID.generate());
				process.addChild(sharingbatching);
				sharingbatching.setProcess(process);
			}
	...
	/**
	 * Retrieves all sharingbatching elements.
	 * 
	 * @param elements
	 *            The list of {@link BPMNElement}.
	 * 
	 * @param Sharingbatching
	 *            The resulting list of {@link Sharingbatching}
	 */
	private void getAllSharingbatchings(List<BPMNElement> elements,
			List<Sharingbatching> sharingbatchings) {
		for (BPMNElement element : elements) {
			if (element.getNode() instanceof Lane
					|| element.getNode() instanceof SubProcess) {
				getAllSharingbatchings(this.getChildElements(element), sharingbatchings);
				continue;
			}

			if (element.getNode() instanceof Sharingbatching) {
				sharingbatchings.add((Sharingbatching) element.getNode());
			}
		}
	}
	...
	/**
	 * @return All {@link ConfigurableTask} contained in the diagram.
	 */

	private List<ConfigurableTask> getAllConfigurableTasks() {
		ArrayList<ConfigurableTask> activities = new ArrayList<ConfigurableTask>();
		for (BPMNElement element : this.bpmnElements.values()) {
			if (element.getNode() instanceof ConfigurableTask)
				activities.add((ConfigurableTask) element.getNode());
		}
	...
	private void addNode(Process process, BPMNElement element,
		...
		// Set id of process if there is one applied for round-tripping
		// scenarios
		...
		if (node instanceof BoundaryEvent) {
			if (((BoundaryEvent) node).getAttachedToRef() != null) {
				addNode(process,
						this.getBpmnElementForNode(((BoundaryEvent) node)
	...
	private void insertSubprocessShapes(SubProcess subProcess) {
		for (FlowElement flowEle : subProcess.getFlowElement()) {
			if (flowEle instanceof FlowNode
				...
				this.definitions.getFirstPlane().getDiagramElement()
						.add(this.bpmnElements.get(flowEle.getId()).getShape());
			}
		...
		for (Artifact a : subProcess.getArtifact()) {
			if (this.bpmnElements.get(a.getId()) != null) {
				this.definitions.getFirstPlane().getDiagramElement()
						.add(this.bpmnElements.get(a.getId()).getShape());
			}
		}
	}
	...
	private boolean insertProcessShapes(Process process) {
		...
		for (Lane lane : process.getAllLanes()) {
			this.definitions.getFirstPlane().getDiagramElement()
					.add(this.bpmnElements.get(lane.getId()).getShape());
		}
		...
		if (process.getIoSpecification() != null) {
			for (DataInput dataInput : process.getIoSpecification()
					.getDataInput()) {
				this.definitions
						.getFirstPlane()
						.getDiagramElement()
						.add(this.bpmnElements.get(dataInput.getId())
								.getShape());
			}

			for (DataOutput dataOutput : process.getIoSpecification()
					.getDataOutput()) {
				this.definitions
						.getFirstPlane()
						.getDiagramElement()
						.add(this.bpmnElements.get(dataOutput.getId())
								.getShape());
			}
		}
	...
	private void insertProcessesIntoDefinitions() {
		for (Process process : this.processes) {
			...
			// /* Insert Data Objects form IOSpecifications */
			// if(process.getIoSpecification() != null) {
			// for(DataInput dataInput :
			// process.getIoSpecification().getDataInput()) {
			// this.definitions.getFirstPlane().getDiagramElement().add(
			// this.bpmnElements.get(dataInput.getId()).getShape());
			// }
			//
			// for(DataOutput dataOutput :
			// process.getIoSpecification().getDataOutput()) {
			// this.definitions.getFirstPlane().getDiagramElement().add(
			...
			/*
			 * Insert just the shapes of the temporary processes, because their
			 * process elements are already connected to and so inserted in the
			 * definitions
			 */
	...
	private void insertCollaborationElements() {
		...
		if (this.collaboration != null) {
			...
			for (Artifact a : getCollaboration().getArtifact()) {
				this.definitions.getFirstPlane().getDiagramElement()
						.add(bpmnElements.get(a.getId()).getShape());
			}
	 ...
	 * @return
	 */
	private Participant retrieveParticipantForMessage(
			ChoreographyActivity choreoActivity, Message m) {
		if (choreoActivity.getParticipantRef().isEmpty()) {
			return null;
		}

		BPMNShape messageShape = (BPMNShape) bpmnElements.get(m.getId())
				.getShape();
		double minDistance = -1.0;
		Participant returnParticipant = null;

		for (Participant p : choreoActivity.getParticipantRef()) {
			BPMNShape participantShape = (BPMNShape) bpmnElements
					.get(p.getId()).getShape();
			double distance = DiagramHelper.calculateCenterDistance(
					messageShape.getBounds(), participantShape.getBounds());
	...
	private void insertChoreographyProcessesIntoDefinitions() {
		for (Process p : this.processes) {
			...
			for (FlowElement flowEle : p.getFlowElement()) {
				...
				if (!(flowEle instanceof Edge) && !(flowEle instanceof Message)) {
					this.definitions
							.getFirstPlane()
							.getDiagramElement()
							.add(this.bpmnElements.get(flowEle.getId())
									.getShape());
					...
					if (flowEle instanceof ChoreographyActivity) {
						for (Participant participant : ((ChoreographyActivity) flowEle)
							...
							this.definitions
									.getFirstPlane()
									.getDiagramElement()
									.add(this.bpmnElements.get(
											participant.getId()).getShape());
						}
					}
				}
					...
					for (String id : idList) {
						...
						if (bpmnEl != null) {
							this.definitions.getFirstPlane()
									.getDiagramElement().add(bpmnEl.getShape());
						}
					}
				}
			...
			for (Artifact artifact : p.getArtifact()) {
				...
				/* DI element */
				this.definitions.getFirstPlane().getDiagramElement()
						.add(bpmnElements.get(artifact.getId()).getShape());
			}
		...
		try {
			...
			for (int i = 0; i < a.length(); i++) {
				...
				if ((localpart != null || ns != null || prefix != null)
						&& value != null) {
					this.definitions.getOtherAttributes().put(
							new QName((ns != null ? ns : ""),
	...
	private void setIOSpecification() {
		...
		for (ConfigurableTask t : this.getAllConfigurableTasks())
			t.determineIoSpecification();
	}
		...
		for (BPMNElement element : this.bpmnElements.values()) {
			if (element.getNode() instanceof Edge) {
				...
				if (element.getNode() instanceof Association
					...
					definitions.getFirstPlane().getDiagramElement()
							.add(element.getShape());
				}

				/* Data Associations */
				if (element.getNode() instanceof DataAssociation) {
					DataAssociation dataAsso = (DataAssociation) element
							.getNode();
					if (dataAsso.getSourceRef() != null
							&& dataAsso.getTargetRef() != null
							&& (dataAsso.getSourceRef() instanceof Activity || dataAsso
									.getTargetRef() instanceof Activity)) {
						definitions.getFirstPlane().getDiagramElement()
								.add(element.getShape());
					}
				}
	...
	public Definitions getDefinitionsFromDiagram()
		...
		this.handleElasticitys();
		this.handleSharingbatchings();
	...
	private void ensureSignavioUUIDStyle() {
		// if(this.configuration != null &&
		// this.configuration.ensureSignavioStyle) {
		for (BPMNElement bpmnEl : bpmnElements.values()) {
			if (!SignavioIDChecker
					.isValidID(bpmnEl.getId(), this.configuration)) {
				/*
	...
	/**
	 * Retrieves the called elements and puts them into the root elements
	 * section as global defined elements.
	 */
	private void putCalledElementsIntoDefinitions() {
		for (BPMNElement bpmnEl : bpmnElements.values()) {
			if (bpmnEl.getNode() instanceof CallingElement) {
				this.definitions.getRootElement()
						.addAll(((CallingElement) bpmnEl.getNode())
								.getCalledElements());
			}
		}
	}
	 ...
	 * Every data input and output element is moved to the
	 * {@link InputOutputSpecification} and an additional data input/output
	...
	private void createProcessIOSpec() {
		...
		for (Process p : this.processes) {
			...
			if (ioSpec.getDataInput().size() > 0
					|| ioSpec.getDataOutput().size() > 0) {
		...
		for (BPMNElement bpmnElement : bpmnElements.values()) {
			...
			/* Ignore messages on the canvas */
				...
				if (m.getProcess() != null
						&& !m.getProcess().isChoreographyProcess()) {
	...
	private void insertDiagramsForLinkedSubprocesses() {
		for (BPMNElement bpmnel : this.bpmnElements.values()) {
			...
			if (bpmnel.getNode() instanceof ContainerElement) {
				List<BPMNDiagram> diagramList = createBPMNDiagramForContainerElement((ContainerElement) bpmnel
						.getNode());
			...
			if (bpmnel.getNode() instanceof CallChoreography) {
				CallChoreography callChoreo = (CallChoreography) bpmnel
						.getNode();

				if (callChoreo.getCalledChoreographyRef() != null
						&& !(callChoreo.getCalledChoreographyRef() instanceof GlobalChoreographyTask)) {
					this.definitions.getRootElement().add(
							callChoreo.getCalledChoreographyRef());
				}
			}

			/*
			 * Insert elements referenced by a collapsed call activity sub
			 * process
			 */
			if (bpmnel.getNode() instanceof CallActivity
					&& ((CallActivity) bpmnel.getNode())._diagramElement != null) {
				this.definitions.getDiagram().add(
						((CallActivity) bpmnel.getNode())._diagramElement);
			}
		}
	}
	...
	private List<BPMNDiagram> createBPMNDiagramForContainerElement(
		...
		if (container._getDiagramElements().size() != 0) {
			...
			diagram.getBPMNPlane().getDiagramElement()
					.addAll(container._getDiagramElements());
	...
	/**
	 * Collects the namespace definitions provided by xml elements from external
	 * sources like Activiti
	 */
	...
	/**
	 * Checks if all process IDs are valid and generates new ones in case of a
	 * negative check.
	 */
